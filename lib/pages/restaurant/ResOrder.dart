...
class _HomePageState extends State<ResOrderPage> {
  ...
  Future<void> _loadAndCompareAddresses() async {
    ...
    try {
      ...
      await _loadRestaurantAddress(res_id, firebaseResCoordinate);
    } catch (e) {
  ...
  Future<void> _loadRestaurantAddress(
      int res_id, String? firebaseResCoordinate) async {
    if (!mounted) return;

    try {
      // โหลดข้อมูลร้านจาก SQL
      final resInfo = await http.get(
        Uri.parse("$url/db/loadResInfo/$res_id"),
      );
